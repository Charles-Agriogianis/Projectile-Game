...
public class CHarController : MonoBehaviour {
    ...
    void Move() {
        ...
        if (isGrounded) {
            Vector3 newDirection = new Vector3(Input.GetAxis("joy_0_axis_0"), 0, Input.GetAxis("joy_0_axis_1"));
            Vector3 direction = new Vector3(oldDirection.x + newDirection.x, 0, oldDirection.z + newDirection.z);
            if (isGrounded)
            {
                horizontalAugment = horizontal * moveSpeed * Time.deltaTime * Input.GetAxis("joy_0_axis_0") + oldHorizontal;
            ...
            if (isGrounded)
            {
                verticalAugment = vertical * moveSpeed * Time.deltaTime * Input.GetAxis("joy_0_axis_1") + oldVertical;
            ...
            horizontalMovement1 = horizontal * moveSpeed * Time.deltaTime * Input.GetAxis("joy_0_axis_0") + oldHorizontal - horizontalAugment + (.001f * (collisionHorizMovement));// - impulseSumHor));
            verticalMovement1 = vertical * moveSpeed * Time.deltaTime * Input.GetAxis("joy_0_axis_1") + oldVertical - verticalAugment + (.001f * (collisionVertMovement));// - impulseSumVer));
    ...
    void Jump() {
        if (isGrounded && (Input.GetAxis("joy_0_axis_5") >= .1 || Input.GetAxis("joy_0_axis_5") <= -.1)) {
    ...
    void Fire() {
        if (Time.time >= timeStamp1) {
            if (Input.GetAxis("joy_0_axis_6") >= .1) {
                Debug.Log("fire");
                chargeAmount += 0.2f;
                if (chargeAmount > 20f) {
                    chargeAmount = 20f;
                ...
                Transform bullet = Instantiate(Bullet1, transform.position, transform.rotation);
                ...
                bulletImpulse1.Add((new Vector3(Input.GetAxis("joy_0_axis_0"), 0, Input.GetAxis("joy_0_axis_1")) * chargeAmount));
    ...
    void OnCollisionEnter(Collision col)
    {
        if (col.gameObject.name == "Bullet2(Clone)")
        {
            ...
            Vector3 heading = Vector3.Normalize(Player2Controller.horizontalMovement1 + Player2Controller.verticalMovement1);
            transform.forward = heading;
            collisionHorizMovement += 8 * Player2Controller.horizontalMovement1;//colHorMov1[index];
            collisionVertMovement += 8 * Player2Controller.verticalMovement1;//colVerMov1[index];
        ...
        {
            transform.position += .5f * Player2Controller.verticalMovement1;
            transform.position += .5f * Player2Controller.horizontalMovement1;
            oldVertical = .5f * (oldVertical + Player2Controller.verticalMovement1);
            oldHorizontal = .5f * (oldHorizontal + Player2Controller.horizontalMovement1);
        }
    }
}
